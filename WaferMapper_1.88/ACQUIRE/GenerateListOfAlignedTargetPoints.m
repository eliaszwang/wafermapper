...
%% Use additional Crosscorrelation to align
try 
    reAlignTarget = GuiGlobalsStruct.TargetPointParameters.useCrossCorrelation;
catch err
    reAlignTarget = 1;
end
...
%             end
            ...
            if reAlignTarget % use additional cross correlation or use previous alignments
            ...
            %% make large image centered at target point.
            %             %Also grab a 3x larger image centered at same region
            
    
                %% if image exeeds boarders
                imageInfo = imfinfo(ImageFileNameStr);
                maxH = imageInfo.Height;
                maxW = imageInfo.Width;
                desiredPixels = {[New_r-(half_h*3+1) New_r+(half_h*3+1)], [New_c-(half_w*3+1) New_c+(half_w*3+1)]};
                actualPixels = {[max(1,New_r-(half_h*3+1)) min(maxH,New_r+(half_h*3+1))],...
                    [max(1,New_c-(half_w*3+1)) min(maxW,New_c+(half_w*3+1))]};
                NewImage_x3LargerROI_shifted = zeros(diff(desiredPixels{1})+1,diff(desiredPixels{2})+1,'uint8');
                smallerImage = imread(ImageFileNameStr,'PixelRegion',actualPixels);
                NewImage_x3LargerROI_shifted  =  NewImage_x3LargerROI_shifted  + uint8(mode(single(smallerImage(:))));
                
                
                shiftY = actualPixels{1}(1) - desiredPixels{1}(1) ;
                shiftX = actualPixels{2}(1) - desiredPixels{2}(1);
                
                
                NewImage_x3LargerROI_shifted(1+ shiftY:shiftY+size(smallerImage,1),1+shiftX:shiftX+size(smallerImage,2)) = smallerImage;
